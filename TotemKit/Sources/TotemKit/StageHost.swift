...
/// Owner-hosted arbitration for a stage, no server involved: the owner's
/// device is the only place the reducer runs, and everyone else forwards
/// actions to it and renders what it broadcasts back.
///
/// Pure. Every entry point takes the local stage and the local user and
/// answers with effects. A `broadcast` is both the new local stage and what
/// participants are sent; the only other thing that changes the local copy is
/// accepting a broadcast from the owner (`accepts`).
public enum StageHost {
    public enum Effect: Hashable, Sendable {
        /// Owner to everyone, the local user included: this is the stage now.
        case broadcast(Stage?, actorID: UUID?)
        /// Owner to one participant, quietly: what is actually on the stage.
    ...
    /// runs here; otherwise the action is forwarded to the owner.
    ...
    /// stage; otherwise the sender is re-synced, which names the real owner.
    ...
    private static func host(_ action: StageAction, expectedVersion: Int?, by actorID: UUID,
        ...
        case .unchanged:
            return []
        case .cleared:
            // Nobody chose this, so it goes out unattributed and posts no notice.
            return [.broadcast(nil, actorID: nil)]
        case .rejected:
            // Only a peer can be holding a stale copy.
            return resyncing.map { [.resync(current, to: $0)] } ?? []
        }
    }

    /// Whether a stage broadcast from `senderID` replaces the local copy. Only
    /// the owner's word counts, an empty stage is anyone's to fill, and when
    /// two claims collide the lower user ID wins. Every participant applies
    /// that rule alone, so all copies converge without another message.
    ...
    /// Answers a request for the current stage. Only the owner answers;
    /// silence means there is no stage.
